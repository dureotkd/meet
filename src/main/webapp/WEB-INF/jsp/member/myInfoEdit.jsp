	...
	<form action="doMyInfoEdit" method="POST"
		...
		<div class="column">
			<input type="hidden" name="fileIdsStr" /> <input class="inputItem"
				...
				class="inputItem" type="email" name="email"
				value="${loginedMember.email}" /> <label for="">이름</label> <input
				class="inputItem" type="text" name="name"
				value="${loginedMember.name}" /> <label for="" class="label">사용자
				이름</label> <span class="mini red">*사용자 이름은 14일 안에 두 번만 변경할 수 있습니다.</span> <input
				class="inputItem" type="text" name="nickname"
				value="${loginedMember.nickname}" /> <label for="">소개</label>
			<textarea class="inputItem Big" type="text" name="introduce" id=""
				cols="30" rows="10" name="introduce">${loginedMember.introduce}</textarea>
			<div class="row sp-b">
				<input type="submit" class="submit2 margin-bottom30 w100px "
					value="저장" /> <a href="./showDisAbledForm"
					class="blue size14 size" onclick="">계정을 일시적으로 비활성화</a>
			</div>
		</div>
	</form>
</div>
